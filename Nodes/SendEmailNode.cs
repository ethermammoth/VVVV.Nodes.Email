...
namespace VVVV.Nodes
{
    ...
    public class SendEmailNode : IPluginEvaluate
    {
        ...
        IDiffSpread<bool> FUseDefaultCredentials;
        ...
        public void Evaluate(int SpreadMax)
        {
            if (FPinInDoSend.IsChanged)
            {
                SpreadMax = SpreadUtils.SpreadMax(FPinInAttachment, FPinInDoSend, FPinInEmailEncoding, 
                    ...
                    FPinInPort, FPinInSSL, FPinInSubject, FPinInTo, FPinInToName, FUseDefaultCredentials, FPinInUsername);
                ...
                for (int i = 0; i < SpreadMax; i++)
                {
                    if (FPinInDoSend[i])
                    {
                        ...
                        if (Username.Length > 0 && Pwd.Length > 0)
                        {
                            ...
                            EmailClient.UseDefaultCredentials = FUseDefaultCredentials[i];
